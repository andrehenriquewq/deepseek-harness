...
行内文本一律由推导得出，绝不来自模型撰写。已归类的变体用标题和图标点明动作，把关键参数放进摘要槽；未归类的行则把摘要取自工具自己声明的 call view 标题，没有声明时回退为 wire 名称加上它的第一个字符串参数。运行中的父调用——一个 `run_code` 程序——用它最新一个尚未结算的 Code Dispatch 子调用来报告当下在做什么，标签取自该子调用声明的 view；子调用的工具没有声明时，取它自己那一行同样由参数推导出的摘要。尚无子调用的父调用，以及所有已结算的父调用，都保留自己的摘要。
