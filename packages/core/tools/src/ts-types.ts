...
\`run_code\` takes one required argument, \`code\` — the body of an async TypeScript function (erasable syntax only — no \`enum\` or namespaces; type annotations are advisory, the code runs type-stripped) — plus an optional \`description\`, a short summary of what the program does. Inside the program:
