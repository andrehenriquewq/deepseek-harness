...
import { gitRepositoryStateSchema } from './git-state.schema.ts'
...
export const hostFrameSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('host/git-state-changed'), path: z.string().min(1), state: gitRepositoryStateSchema }),
