...
工具事件在原始事件旁携带一个可选的 `view`，`session/event` mux 帧与 `session.history` 条目都是如此：它是工具自己的 `presentCall`/`presentResult` 产出的渲染意图，通过投递时已注册的 presenter 计算得出，绝不入日志。有四类事件能解析出 `view`：原生调用的 `tool/call` 与 `tool/result`（结果的 presenter 需要该调用已解析的参数，由按会话的实时调用表或页内回扫提供），以及一次 `run_code` 子调用的 `tool/code-dispatch-start` 与 `tool/code-dispatch`——后者无需配对，因为这两个事件都携带 bridge 派发时那份 JSON 规范化参数。于是子调用能拿到与原生调用完全相同的、由工具自己拥有的卡片，运行中程序的活动可以从它调用过的工具读出，而不必依赖模型为自己的调用写的说明文字。子派发的结算事件不带 `meta`，因此从结果元数据投影卡片的 presenter 在那里返回空，子调用保留通用正文。配对缺失、presenter 缺失、参数无法解析以及 presenter 抛错都会软降级为不带 `view`，也就是客户端有文档记载的通用卡片默认行为。
