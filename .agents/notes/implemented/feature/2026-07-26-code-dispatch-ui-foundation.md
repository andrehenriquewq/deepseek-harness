# Agent Note: Code Mode UI foundation — run_code presentation and native-parity dispatch logging

Status: implemented

English | [中文](2026-07-26-code-dispatch-ui-foundation.zh.md)

> Scope: the host-side changes that let a UI render a Code Mode turn with the same fidelity as native tool calls — the foundation the other Code Mode UI notes build on. The [Code Mode foundation](2026-06-15-code-mode.md) owns the transport design; this note owns the call presentation, the full-content `tool/code-dispatch` payload, and the temporary `DSH_TOOLS_MODE` enablement switch for the `dsh` config tree. The [preamble-only tool-call decision](../bug-fix/2026-08-22-preamble-only-tool-calls.md) owns the removal of the presentation-only argument originally recorded here.

## Problem

A `run_code` turn was opaque in every product surface. The call card's title was the raw program text, unreadable at row width. The `tool/code-dispatch` event carried only a 200-char, cwd-normalized `resultSummary` of each sub-call, so no UI could ever show what a sub-call actually returned: the web conversation view ([chat sub-call rows](2026-07-26-code-mode-chat-subcall-rows.md)) renders sub-calls through the exact components that render native `tool/result` cards, and a bounded summary cannot feed a native-parity card. And the `dsh web` composition had no way to enable Code Mode at all — the `tools` row pinned the schema default and the runtime was absent from the tree.

## Decision

Three changes, one per obstacle:

1. **`run_code` presents the program separately from its title.** `presentCall` puts the complete program in `rawInput` and derives the title from its first non-blank line. Every surface — TUI card, ACP title, web row — gets a bounded label without a separate model-authored argument.
