...
与终端卡片的不对称是刻意的，继承自后端约定：`terminalCardModel` 同时读 `callView` 和 `resultView`，因为命令与 cwd 在调用时就存在；`searchCardModel` 只读 `resultView`，因为搜索的匹配或路径只在执行后存在。因此运行中的搜索行只显示摘要，没有卡片。
