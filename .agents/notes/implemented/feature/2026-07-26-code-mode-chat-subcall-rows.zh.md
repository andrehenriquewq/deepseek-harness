...
> 范围：Web chat 视图如何渲染一个 `run_code` 轮次，即 Code Mode UI 栈的客户端侧部分，构建在[宿主侧基础](2026-07-26-code-dispatch-ui-foundation.zh.md)及其携带完整内容的 `tool/code-dispatch` 事件之上。本篇所依托的 slot 模型归 [toolview 溶解](../architecture/2026-07-23-toolview-dissolution.zh.md)所有。
...
- **`run_code` 的呈现**：`code` 行变体（分类器映射 `run_code → code`、图标 `IconCodeOutline16`）以程序的首个非空行作摘要，展开后显示完整程序（在 markdown 代码块的填充底色上以等宽字体呈现），而非参数的 JSON 封装。该标题推导归[仅含前言的工具调用决策](../bug-fix/2026-08-22-preamble-only-tool-calls.zh.md)所有。
