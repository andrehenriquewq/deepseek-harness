...
- **`run_code` presentation**: a `code` row variant (classifier `run_code → code`, `IconCodeOutline16`) summarizes with the program's first non-blank line and expands to the complete program (monospace on the markdown code-block fill) rather than the args JSON envelope. The [preamble-only tool-call decision](../bug-fix/2026-08-22-preamble-only-tool-calls.md) owns this title derivation.
