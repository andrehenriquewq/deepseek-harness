...
- **Presentation is the harness's job, not the model's.** Never add a parameter whose only purpose is a UI label. A field the model can see is a field it will fill, and a Gemini-family model filled exactly that field INSTEAD of the payload, killing whole turns ([preamble-only calls](../../.agents/notes/implemented/bug-fix/2026-08-22-preamble-only-tool-calls.md)). Derive the label from the operational arguments your tool already needs.
- **Your presenters also run for Code Mode sub-calls.** `tool/code-dispatch-start` and `tool/code-dispatch` resolve your `presentCall`/`presentResult` from the arguments the bridge dispatched, so a `run_code` program calling your tool renders the same card a model calling it natively does, and the parent `run_code` row reports your call as its current activity. The sub-dispatch settle carries no `meta`, so a result view projected from `presentationMeta` is unavailable there and the sub-call falls back to the generic body — the call view is the one that always applies.
